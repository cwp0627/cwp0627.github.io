...
function updateGUI() {
    document.getElementById("population").textContent = "Population: " + format(population)
    for (let i = 0; i < 10; i++) {
        let g = generators[i]
        ...
        if (g.cost > population) document.getElementById("gen" + (i + 1)).classList.add("locked")
        else document.getElementById("gen" + (i + 1)).classList.remove("locked")
    }
}

function productionLoop(diff) {
    population += generators[0].amount * generators[0].mult * diff
    for (let i = 1; i < 10; i++) {
